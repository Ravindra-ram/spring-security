	...
	public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
		return this.firewall.getFirewalledExchange(exchange)
				...
				.onErrorResume(ServerExchangeRejectedException.class,
						(ex) -> this.exchangeRejectedHandler.handle(exchange, ex).then(Mono.empty()));
	}

	private Mono<Void> filterFirewalledExchange(ServerWebExchange firewalledExchange, WebFilterChain chain) {
		return Flux.fromIterable(this.filters).filterWhen((securityChain) -> securityChain.matches(firewalledExchange))
				.next().switchIfEmpty(chain.filter(firewalledExchange).then(Mono.empty()))
				.flatMap((securityChain) -> securityChain.getWebFilters().collectList())
